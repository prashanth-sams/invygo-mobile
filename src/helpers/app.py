...
class App(Driver):
    ...
    def click(self, locator, n=3, **kwargs):
        ...
        {
            'element': lambda x: self.driver.execute_script("arguments[0].scrollIntoView();", App.element(self, locator)),
            'elements': lambda x: self.driver.execute_script("arguments[0].scrollIntoView();", App.elements(self, locator)[kwargs['index']])
    ...
    def wait_until_appear(self, locator, n=3, **kwargs):
        wait = WebDriverWait(self.driver, 25)
        wait.until(EC.visibility_of_element_located(locator))
    ...
    def get_text(self, locator, **kwargs):
        """
        custom wrapped get_text method
        -> wait for element until display
        -> element(s)
        """
        App.is_displayed(self, locator, True)
        ...
        return {
            'element': lambda x: App.element(self, locator).text,
            'elements': lambda x: App.elements(self, locator)[kwargs['index']].text
        }[keyword_check(kwargs)]('x')
    ...
    def assert_text(self, locator, text, n=20, **kwargs):
        ...
        while n > 1:
            try:
                if len(kwargs) == 0:
                    self.logger.info(f"assert_text: {App.element(self, locator).text} == {text}")
